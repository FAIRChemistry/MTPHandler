...
TEKAN_DEVICE_IDENTIFIER = "Device: infinite 200Pro"
...
def _parse_kinetic_metadata(
    ...
    time_measured_str = (
        meta_df.loc["Start Time:"].dropna(axis=1, how="all").values[0][0]
    )
    time_measured = datetime.strptime(time_measured_str, "%d.%m.%Y %H:%M:%S")

    # Validate wavelength data
    try:
        wavelength = meta_df.iat[-6, 3]
...
def _parse_kinetic_data(
    ...
    # check for index "Movement" and cut df thow rows prior to that
    movement_index = data_df.index.get_loc("Movement")
    data_df = data_df.iloc[: movement_index - 2, :].T

    data_df = data_df[~data_df.index.isna()]
    # print(data_df, "diese df")
...
def read_tekan_infinity(path: str, ph: Optional[float]) -> Plate:
    """
    ...
    if not df.iloc[0, 0] == TEKAN_DEVICE_IDENTIFIER:
